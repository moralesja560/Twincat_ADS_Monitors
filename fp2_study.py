...
def write_log(i_r1_amp,i_r1_vlt,i_r1_spd,i_r2_amp,i_r2_vlt,i_r2_spd,i_f_motor_amp,i_f_motor_spd,i_f_motor_sp,i_f_motor_p,i_cad_amp,i_cad_vlt,i_cad_spd,i_cad_speed):
	...
	pd_ruta = str(mis_docs)+ r"\registro_fp2_total.csv"
...
def PLC_comms1(PLC_1_queue_i,PLC_1_queue_o,plc1_ip,plc1_netid):
	...
	try:
		...
		#FP roller 1 vlt
		#FP roller 1 spd
		roller1_amp = plc1.get_handle('.IW_FSA_Tragrollenantrieb1_Ist_Strom_100mA_INT')
		roller1_volt = plc1.get_handle('.IW_FSA_Tragrollenantrieb1_Ist_Spannung_INT')
		roller1_spd = plc1.get_handle('.IW_FSA_Tragrollenantrieb1_Ist_Drehzahl_INT')	
		print("F1")
		#FP roller 2 amp
		#FP roller 2 vlt
		#FP roller 2 spd
		roller2_amp = plc1.get_handle('.IW_FSA_Tragrollenantrieb2_Ist_Strom_100mA_INT')
		roller2_volt = plc1.get_handle('.IW_FSA_Tragrollenantrieb2_Ist_Spannung_INT')
		roller2_spd = plc1.get_handle('.IW_FSA_Tragrollenantrieb2_Ist_Drehzahl_INT')		
		print("F2")
		# FP filter spd
		# FP filter pressure
		# FP filter setpoint
		# FP filter amp
		motor_amp = plc1.get_handle('.IW_FSA_Filterventilator_Ist_Strom_100mA_INT')
		real_spd = plc1.get_handle('.IW_FSA_Filterventilator_Ist_Drehzahl_INT')	
		spd_setpoint = plc1.get_handle('.IW_FSA_Filterventilator_Soll_Drehzahl_Mot_WORD')
		pressure = plc1.get_handle('.TP_IW_FSA_Filterventilator_Luftdruck_in_pa_INT')
		print("F3")
		# FP chain amp
		# FP chain vlt
		# FP spring rate
		# FP chain speed
		chain_amp = plc1.get_handle('.TP_IW_FSA_Mitnehmerkette_Ist_Strom_100mA_INT')
		chain_volt = plc1.get_handle('.IW_FSA_Mitnehmerkette_Ist_Spannung_INT')
		chain_spd = plc1.get_handle('.IW_FSA_Mitnehmerkette_Ist_Drehzahl_INT')	
		chain_speed = plc1.get_handle('.TP_IW_FSA_Mitnehmerkette_Geschwindigkeit_auto_Calc_INT')
		print("F4")
...
def aux_PLC_comms(plc_address_aux,plc_netid_aux):
	...
	while True:
		try:
			...
			# FP spring rate
			# FP chain speed
			chain_amp = plc1.get_handle('.IW_FSA_Mitnehmerkette_Ist_Strom_100mA_INT')
			chain_volt = plc1.get_handle('.IW_FSA_Mitnehmerkette_Ist_Spannung_INT')
			chain_spd = plc1.get_handle('.IW_FSA_Mitnehmerkette_Ist_Drehzahl_INT')	
			chain_speed = plc1.get_handle('.TP_IW_FSA_Mitnehmerkette_Geschwindigkeit_auto_Calc_INT')
			print("F4")
